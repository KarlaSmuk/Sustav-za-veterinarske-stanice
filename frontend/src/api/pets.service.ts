...
export const getPetsByOwnerId = async (ownerId: string) => {
    try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/pet/owner/` + ownerId);
...
export const getPetById = async (petId: string) => {
    try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/pet/` + petId);
        return response.data;
    } catch (error) {
